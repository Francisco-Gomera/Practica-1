...
async def get_preferences(user_id: int):
    try:
        ...
        mydb_conn = await mydb.get_connection()         
        mycursor = mydb_conn.cursor()
        query = "SELECT cancion, artista FROM preferencias WHERE id_usuario = %s"
    ...
    if not rows:
        return JSONResponse(content={"message": "No se encontraron preferencias"}, status_code=404)
    ...
    return JSONResponse(content={"usuario_id": user_id, "preferencias": resultados_spotify}, status_code=200)
